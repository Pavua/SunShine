...
#undef _CONVERT_2_ARG_
  ...
  void map_int_int_f(std::unordered_map<std::string, std::string> &vars, const std::string &name, std::unordered_map<int, int> &input) {
    ...
    int x = 0;
    while (static_cast<size_t>(x) < list.size()) {
      auto key = list[static_cast<size_t>(x++)];
      auto val = list[static_cast<size_t>(x++)];
  ...
  int parse(int argc, char *argv[]) {
    ...
    try {
      ...
      BOOST_LOG(log_error) << "Failed to apply config: "sv << err.what();
    ...
    if (!config_loaded && !shortcut_launch) {
      BOOST_LOG(log_error) << "To relaunch Sunshine successfully, use the shortcut in the Start Menu. Do not run Sunshine.exe manually."sv;
    ...
    if (shortcut_launch) {
      if (!service_ctrl::is_service_running()) {
        ...
        if (!ShellExecuteExW(&shell_exec_info)) {
          ...
          BOOST_LOG(log_error) << "Failed executing shell command: " << winerr << std::endl;
          return 1;
        }
