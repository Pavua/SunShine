...
#pragma pack(pop)
  ...
  public:
    ...
    void read() {
      if (begin == std::end(msg_buf) || (session->rtsp_cipher && begin + sizeof(encrypted_rtsp_header_t) >= std::end(msg_buf))) {
        BOOST_LOG(log_error) << "RTSP: read(): Exceeded maximum rtsp packet size: "sv << msg_buf.size();
    ...
    static void handle_read_encrypted_header(std::shared_ptr<socket_t> &socket, const boost::system::error_code &ec, std::size_t bytes) {
      ...
      auto sock_close = util::fail_guard([&socket]() {
        ...
        if (ec) {
          BOOST_LOG(log_error) << "RTSP: handle_read_encrypted_header(): Couldn't close tcp socket: "sv << ec.message();
        }
      });

      if (ec || bytes < sizeof(encrypted_rtsp_header_t)) {
        BOOST_LOG(log_error) << "RTSP: handle_read_encrypted_header(): Couldn't read from tcp socket: "sv << ec.message();
      ...
      if (!header->is_encrypted()) {
        BOOST_LOG(log_error) << "RTSP: handle_read_encrypted_header(): Rejecting unencrypted RTSP message"sv;
      ...
      if (socket->begin + sizeof(*header) + payload_length >= std::end(socket->msg_buf)) {
        BOOST_LOG(log_error) << "RTSP: handle_read_encrypted_header(): Exceeded maximum rtsp packet size: "sv << socket->msg_buf.size();
    ...
    static void handle_read_encrypted_message(std::shared_ptr<socket_t> &socket, const boost::system::error_code &ec, std::size_t bytes) {
      ...
      auto sock_close = util::fail_guard([&socket]() {
        ...
        if (ec) {
          BOOST_LOG(log_error) << "RTSP: handle_read_encrypted_message(): Couldn't close tcp socket: "sv << ec.message();
        }
      });
      ...
      if (ec || bytes < payload_length) {
        BOOST_LOG(log_error) << "RTSP: handle_read_encrypted(): Couldn't read from tcp socket: "sv << ec.message();
      ...
      if (socket->session->rtsp_cipher->decrypt(std::string_view {(const char *) header->tag, sizeof(header->tag) + bytes}, plaintext, &iv)) {
        BOOST_LOG(log_error) << "Failed to verify RTSP message tag"sv;
      ...
      if (auto status = parseRtspMessage(req.get(), (char *) plaintext.data(), plaintext.size())) {
        BOOST_LOG(log_error) << "Malformed RTSP message: ["sv << status << ']';
    ...
    void read_plaintext_payload() {
      if (begin == std::end(msg_buf)) {
        BOOST_LOG(log_error) << "RTSP: read_plaintext_payload(): Exceeded maximum rtsp packet size: "sv << msg_buf.size();
    ...
    static void handle_plaintext_payload(std::shared_ptr<socket_t> &socket, const boost::system::error_code &ec, std::size_t bytes) {
      ...
      auto sock_close = util::fail_guard([&socket]() {
        ...
        if (ec) {
          BOOST_LOG(log_error) << "RTSP: handle_plaintext_payload(): Couldn't close tcp socket: "sv << ec.message();
        }
      });

      if (ec) {
        BOOST_LOG(log_error) << "RTSP: handle_plaintext_payload(): Couldn't read from tcp socket: "sv << ec.message();
    ...
    static void handle_read_plaintext(std::shared_ptr<socket_t> &socket, const boost::system::error_code &ec, std::size_t bytes) {
      ...
      if (ec) {
        BOOST_LOG(log_error) << "RTSP: handle_read_plaintext(): Couldn't read from tcp socket: "sv << ec.message();

        boost::system::error_code ec;
        socket->sock.close(ec);

        if (ec) {
          BOOST_LOG(log_error) << "RTSP: handle_read_plaintext(): Couldn't close tcp socket: "sv << ec.message();
        }
  ...
  public:
    ...
    void handle_accept(const boost::system::error_code &ec) {
      if (ec) {
        BOOST_LOG(log_error) << "Couldn't accept incoming connections: "sv << ec.message();
    ...
    void session_clear(uint32_t launch_session_id) {
      ...
      if (launch_session) {
        if (launch_session->id != launch_session_id) {
          BOOST_LOG(log_error) << "Attempted to clear unexpected session: "sv << launch_session_id << " vs "sv << launch_session->id;
        } else {
          launch_event.pop();
        }
      }
  ...
  int send(tcp::socket &sock, const std::string_view &sv) {
    ...
    while (bytes_send != sv.size()) {
      ...
      if (ec) {
        BOOST_LOG(log_error) << "RTSP: Couldn't send data over tcp socket: "sv << ec.message();
        return -1;
      }
    }
  ...
  void cmd_announce(rtsp_server_t *server, tcp::socket &sock, launch_session_t &session, msg_t &&req) {
    ...
    if (encryption_mode == config::ENCRYPTION_MODE_MANDATORY &&
      ...
      BOOST_LOG(log_error) << "Rejecting client that cannot comply with mandatory encryption requirement"sv;
    ...
    if (stream::session::start(*stream_session, sock.remote_endpoint().address().to_string())) {
      BOOST_LOG(log_error) << "Failed to start a streaming session"sv;
