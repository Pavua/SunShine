...
#pragma GCC diagnostic pop
  ...
  if (!log_deinit_guard) {
    BOOST_LOG(log_error) << "Logging failed to initialize"sv;
  }
  ...
  if (!display_device_deinit_guard) {
    BOOST_LOG(log_error) << "Display device session failed to initialize"sv;
  }
  ...
  std::thread session_monitor_thread([&]() {
    ...
    if (!RegisterClassA(&wnd_class)) {
      ...
      BOOST_LOG(log_error) << "Failed to register session monitor window class"sv << std::endl;
      return;
    }
    ...
    if (!wnd) {
      BOOST_LOG(log_error) << "Failed to create session monitor window"sv << std::endl;
      return;
    }
  ...
  if (!platf_deinit_guard) {
    BOOST_LOG(log_error) << "Platform failed to initialize"sv;
  }

  auto proc_deinit_guard = proc::init();
  if (!proc_deinit_guard) {
    BOOST_LOG(log_error) << "Proc failed to initialize"sv;
  }
  ...
  if (video::probe_encoders()) {
    BOOST_LOG(log_error) << "Video failed to find working encoder"sv;
  }
