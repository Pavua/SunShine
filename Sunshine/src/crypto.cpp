...
namespace crypto {
  ...
  namespace cipher {
    ...
    int gcm_t::decrypt(const std::string_view &tagged_cipher, std::vector<std::uint8_t> &plaintext, aes_t *iv) {
      ...
      if (EVP_DecryptUpdate(decrypt_ctx.get(), plaintext.data(), &update_outlen, (const std::uint8_t *) cipher.data(), static_cast<int>(cipher.size())) != 1) {
        return -1;
      }

      if (EVP_CIPHER_CTX_ctrl(decrypt_ctx.get(), EVP_CTRL_GCM_SET_TAG, static_cast<int>(tag.size()), const_cast<char *>(tag.data())) != 1) {
        return -1;
      }
    ...
    int gcm_t::encrypt(const std::string_view &plaintext, std::uint8_t *tag, std::uint8_t *ciphertext, aes_t *iv) {
      ...
      if (EVP_EncryptUpdate(encrypt_ctx.get(), ciphertext, &update_outlen, (const std::uint8_t *) plaintext.data(), static_cast<int>(plaintext.size())) != 1) {
        return -1;
      }
      ...
      if (EVP_CIPHER_CTX_ctrl(encrypt_ctx.get(), EVP_CTRL_GCM_GET_TAG, static_cast<int>(tag_size), tag) != 1) {
        return -1;
      }
    ...
    int ecb_t::encrypt(const std::string_view &plaintext, std::vector<std::uint8_t> &cipher) {
      ...
      // Encrypt into the caller's buffer
      if (EVP_EncryptUpdate(encrypt_ctx.get(), cipher.data(), &update_outlen, (const std::uint8_t *) plaintext.data(), static_cast<int>(plaintext.size())) != 1) {
        return -1;
      }
    ...
    int cbc_t::encrypt(const std::string_view &plaintext, std::uint8_t *cipher, aes_t *iv) {
      ...
      // Encrypt into the caller's buffer
      if (EVP_EncryptUpdate(encrypt_ctx.get(), cipher, &update_outlen, (const std::uint8_t *) plaintext.data(), static_cast<int>(plaintext.size())) != 1) {
        return -1;
      }
  ...
  x509_t x509(const std::string_view &x) {
    ...
    BIO_write(io.get(), x.data(), static_cast<int>(x.size()));
  ...
  pkey_t pkey(const std::string_view &k) {
    ...
    BIO_write(io.get(), k.data(), static_cast<int>(k.size()));
  ...
  std::string_view signature(const x509_t &x) {
    ...
    return {(const char *) asn1->data, static_cast<std::size_t>(asn1->length)};
  }
  ...
  std::string rand(std::size_t bytes) {
    ...
    RAND_bytes((uint8_t *) r.data(), static_cast<int>(r.size()));
  ...
  creds_t gen_creds(const std::string_view &cn, std::uint32_t key_bits) {
    ...
    X509_NAME_add_entry_by_txt(name, "CN", MBSTRING_ASC, (const std::uint8_t *) cn.data(), static_cast<int>(cn.size()), -1, 0);
