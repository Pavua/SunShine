...
namespace audio {
  ...
  void encodeThread(sample_queue_t samples, config_t config, void *channel_data) {
    ...
    while (auto sample = samples->pop()) {
      ...
      int bytes = opus_multistream_encode_float(
        opus.get(), sample->data(), frame_size, std::begin(packet), static_cast<opus_int32>(packet.size()));
      if (bytes < 0) {
        BOOST_LOG(log_error) << "Couldn't encode audio: "sv << opus_strerror(bytes);
        packets->stop();
  ...
  void capture(safe::mail_t mail, config_t config, void *channel_data) {
    ...
    auto init_failure_fg = util::fail_guard([&shutdown_event]() {
      BOOST_LOG(log_error) << "Unable to initialize audio capture. The stream will not have audio."sv;
