...
namespace proc {
  ...
  boost::filesystem::path find_working_directory(const std::string &cmd, boost::process::v1::environment &env) {
...
#endif
    if (parts.empty()) {
      BOOST_LOG(log_error) << "Unable to parse command: "sv << cmd;
    ...
    if (!cmd_path.is_absolute()) {
      ...
      if (cmd_path.empty()) {
        BOOST_LOG(log_error) << "Unable to find executable ["sv << parts.at(0) << "]. Is it in your PATH?"sv;
        return boost::filesystem::path();
      }
    }
  ...
  int proc_t::execute(int app_id, std::shared_ptr<rtsp_stream::launch_session_t> launch_session) {
    ...
    if (iter == _apps.end()) {
      BOOST_LOG(log_error) << "Couldn't find app with ID ["sv << app_id << ']';
      return 404;
    }
    ...
    for (; _app_prep_it != std::end(_app.prep_cmds); ++_app_prep_it) {
      ...
      if (ec) {
        BOOST_LOG(log_error) << "Couldn't run ["sv << cmd.do_cmd << "]: System: "sv << ec.message();
      ...
      if (ret != 0 && ec != std::errc::permission_denied) {
        BOOST_LOG(log_error) << '[' << cmd.do_cmd << "] failed with code ["sv << ret << ']';
        return -1;
      }
    }
