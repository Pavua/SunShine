  ...
  public:
    ...
    bool map_upnp_port(const IGDdatas &data, const urls_t &urls, const std::string &lan_addr, const mapping_t &mapping) {
      ...
      if (err == 714) {  // NoSuchEntryInArray
        ...
        if (!std::strcmp(intClient, lan_addr.c_str())) {
          ...
          if (err) {
            BOOST_LOG(log_error) << "Unable to delete conflicting UPnP port mapping: "sv << err;
            return false;
          }
        }
      } else {
        BOOST_LOG(log_error) << "UPNP_GetSpecificPortMappingEntry() failed: "sv << err;
      ...
      if (err) {
        BOOST_LOG(log_error) << "Failed to map "sv << mapping.port.proto << ' ' << mapping.port.lan << ": "sv << err;
        return false;
      }
    ...
    void upnp_thread_proc() {
      ...
      do {
        ...
        auto status = upnp::UPNP_GetValidIGDStatus(device, &urls, &data, lan_addr);
        if (status != 1 && status != 2) {
          BOOST_LOG(log_error) << status_string(status);
          mapped = false;
          continue;
        }
