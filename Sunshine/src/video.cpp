...
namespace video {
  ...
  namespace {
    ...
    bool allow_encoder_probing() {
      ...
      BOOST_LOG(log_error) << "No display devices are active at the moment! Cannot probe the encoders.";
      return false;
    }
  }  // namespace
  ...
  public:
    int convert(platf::img_t &img) override {
      ...
      if (status < 0) {
        ...
        BOOST_LOG(log_error) << "Couldn't scale frame: "sv << av_make_error_string(string, AV_ERROR_MAX_STRING_SIZE, status);
        return -1;
      }
      ...
      if (frame->hw_frames_ctx) {
        ...
        if (status < 0) {
          char string[AV_ERROR_MAX_STRING_SIZE];
          BOOST_LOG(log_error) << "Failed to transfer image data to hardware frame: "sv << av_make_error_string(string, AV_ERROR_MAX_STRING_SIZE, status);
          return -1;
        }
      }
    ...
    int init(int in_width, int in_height, AVFrame *frame, AVPixelFormat format, bool hardware) {
      ...
      if (status < 0) {
        char string[AV_ERROR_MAX_STRING_SIZE];
        BOOST_LOG(log_error) << "Failed to set SWS options: "sv << av_make_error_string(string, AV_ERROR_MAX_STRING_SIZE, status);
        return -1;
      }
      ...
      if (status < 0) {
        char string[AV_ERROR_MAX_STRING_SIZE];
        BOOST_LOG(log_error) << "Failed to initialize SWS: "sv << av_make_error_string(string, AV_ERROR_MAX_STRING_SIZE, status);
        return -1;
      }
  ...
  public:
    ...
    void invalidate_ref_frames(int64_t first_frame, int64_t last_frame) override {
      BOOST_LOG(log_error) << "Encoder doesn't support reference frame invalidation";
      request_idr_frame();
    }
  ...
  void refresh_displays(platf::mem_type_e dev_type, std::vector<std::string> &display_names, int &current_display_index) {
    ...
    if (display_names.empty() && !old_display_names.empty()) {
      BOOST_LOG(log_error) << "No displays were found after reenumeration!"sv;
    ...
    while (capture_ctx_queue->running()) {
      ...
      switch (status) {
        ...
        case platf::capture_e::interrupted:
          return;
        default:
          BOOST_LOG(log_error) << "Unrecognized capture status ["sv << (int) status << ']';
          return;
      }
    }
  }
  ...
  int encode_avcodec(int64_t frame_nr, avcodec_encode_session_t &session, safe::mail_raw_t::queue_t<packet_t> &packets, void *channel_data, std::optional<std::chrono::steady_clock::time_point> frame_timestamp) {
    ...
    if (ret < 0) {
      ...
      BOOST_LOG(log_error) << "Could not send a frame for encoding: "sv << av_make_error_string(err_str, AV_ERROR_MAX_STRING_SIZE, ret);
    ...
    while (ret >= 0) {
      ...
      if ((frame->flags & AV_FRAME_FLAG_KEY) && !(av_packet->flags & AV_PKT_FLAG_KEY)) {
        BOOST_LOG(log_error) << "Encoder did not produce IDR frame when requested!"sv;
      }
  ...
  int encode_nvenc(int64_t frame_nr, nvenc_encode_session_t &session, safe::mail_raw_t::queue_t<packet_t> &packets, void *channel_data, std::optional<std::chrono::steady_clock::time_point> frame_timestamp) {
    ...
    if (encoded_frame.data.empty()) {
      BOOST_LOG(log_error) << "NvENC returned empty packet";
      return -1;
    }

    if (frame_nr != encoded_frame.frame_index) {
      BOOST_LOG(log_error) << "NvENC frame index mismatch " << frame_nr << " " << encoded_frame.frame_index;
    }
  ...
  std::unique_ptr<avcodec_encode_session_t> make_avcodec_encode_session(
    ...
    if (!video_format[encoder_t::PASSED] || !disp->is_codec_supported(video_format.name, config)) {
      BOOST_LOG(log_error) << encoder.name << ": "sv << video_format.name << " mode not supported"sv;
      return nullptr;
    }

    if (config.dynamicRange && !video_format[encoder_t::DYNAMIC_RANGE]) {
      BOOST_LOG(log_error) << video_format.name << ": dynamic range not supported"sv;
      return nullptr;
    }

    if (config.chromaSamplingType == 1 && !video_format[encoder_t::YUV444]) {
      BOOST_LOG(log_error) << video_format.name << ": YUV 4:4:4 not supported"sv;
      return nullptr;
    }

    auto codec = avcodec_find_encoder_by_name(video_format.name.c_str());
    if (!codec) {
      BOOST_LOG(log_error) << "Couldn't open ["sv << video_format.name << ']';
    ...
    for (int retries = 0; retries < 2; retries++) {
      ...
      if (hardware) {
        ...
        if (platform_formats->avcodec_derived_dev_type != AV_HWDEVICE_TYPE_NONE) {
          ...
          if (err) {
            char err_str[AV_ERROR_MAX_STRING_SIZE] {0};
            BOOST_LOG(log_error) << "Failed to derive device context: "sv << av_make_error_string(err_str, AV_ERROR_MAX_STRING_SIZE, err);
    ...
    if (colorspace_is_hdr(colorspace)) {
      ...
      if (disp->get_hdr_metadata(hdr_metadata)) {
        ...
        BOOST_LOG(log_error) << "Couldn't get display hdr metadata when colorspace selection indicates it should have one";
      }
    }
  ...
  void encode_run(
    ...
    while (true) {
      ...
      if (!requested_idr_frame || images->peek()) {
        if (auto img = images->pop(minimum_frame_time)) {
          ...
          if (session->convert(*img)) {
            BOOST_LOG(log_error) << "Could not convert image"sv;
            return;
          }
      ...
      if (encode(frame_nr++, *session, packets, channel_data, frame_timestamp)) {
        BOOST_LOG(log_error) << "Could not encode video packet"sv;
        return;
      }
  ...
  std::optional<sync_session_t> make_synced_session(platf::display_t *disp, const encoder_t &encoder, platf::img_t &img, sync_session_ctx_t &ctx) {
    ...
    if (session->convert(img)) {
      BOOST_LOG(log_error) << "Could not convert initial image"sv;
      return std::nullopt;
    }
  ...
  int validate_config(std::shared_ptr<platf::display_t> disp, const encoder_t &encoder, const config_t &config) {
    ...
    if (!packet->is_idr()) {
      BOOST_LOG(log_error) << "First packet type is not an IDR frame"sv;
  ...
  int probe_encoders() {
    ...
    if (!config::video.encoder.empty()) {
      ...
      if (chosen_encoder == nullptr) {
        BOOST_LOG(log_error) << "Couldn't find any working encoder matching ["sv << config::video.encoder << ']';
      }
    }
    ...
    if (chosen_encoder == nullptr && (active_hevc_mode >= 2 || active_av1_mode >= 2)) {
      ...
      if (chosen_encoder == nullptr) {
        BOOST_LOG(log_error) << "Couldn't find any working encoder that meets HEVC/AV1 requirements"sv;
      }
    }
  ...
  util::Either<avcodec_buffer_t, int> vaapi_init_avcodec_hardware_input_buffer(platf::avcodec_encode_device_t *encode_device) {
    ...
    if (status < 0) {
      char string[AV_ERROR_MAX_STRING_SIZE];
      BOOST_LOG(log_error) << "Failed to create a VAAPI device: "sv << av_make_error_string(string, AV_ERROR_MAX_STRING_SIZE, status);
      return -1;
    }
  ...
  util::Either<avcodec_buffer_t, int> cuda_init_avcodec_hardware_input_buffer(platf::avcodec_encode_device_t *encode_device) {
    ...
    if (status < 0) {
      char string[AV_ERROR_MAX_STRING_SIZE];
      BOOST_LOG(log_error) << "Failed to create a CUDA device: "sv << av_make_error_string(string, AV_ERROR_MAX_STRING_SIZE, status);
      return -1;
    }
  ...
  util::Either<avcodec_buffer_t, int> vt_init_avcodec_hardware_input_buffer(platf::avcodec_encode_device_t *encode_device) {
    ...
    if (status < 0) {
      char string[AV_ERROR_MAX_STRING_SIZE];
      BOOST_LOG(log_error) << "Failed to create a VideoToolbox device: "sv << av_make_error_string(string, AV_ERROR_MAX_STRING_SIZE, status);
      return -1;
    }
...
namespace video {
  util::Either<avcodec_buffer_t, int> dxgi_init_avcodec_hardware_input_buffer(platf::avcodec_encode_device_t *encode_device) {
    ...
    if (err) {
      char err_str[AV_ERROR_MAX_STRING_SIZE] {0};
      BOOST_LOG(log_error) << "Failed to create FFMpeg hardware device context: "sv << av_make_error_string(err_str, AV_ERROR_MAX_STRING_SIZE, err);
