...
namespace service_ctrl {
  ...
  public:
    ...
    service_controller(DWORD service_desired_access) {
      ...
      if (!scm_handle) {
        ...
        BOOST_LOG(log_error) << "OpenSCManager() failed: "sv << winerr;
        return;
      }
      ...
      if (!service_handle) {
        auto winerr = GetLastError();
        BOOST_LOG(log_error) << "OpenService() failed: "sv << winerr;
        return;
      }
    }
    ...
    bool start_service() {
      ...
      if (!StartServiceA(service_handle, 0, nullptr)) {
        ...
        if (winerr != ERROR_SERVICE_ALREADY_RUNNING) {
          BOOST_LOG(log_error) << "StartService() failed: "sv << winerr;
          return false;
        }
      }
    ...
    bool query_service_status(SERVICE_STATUS &status) {
      ...
      if (!QueryServiceStatus(service_handle, &status)) {
        auto winerr = GetLastError();
        BOOST_LOG(log_error) << "QueryServiceStatus() failed: "sv << winerr;
        return false;
      }
  ...
  bool start_service() {
    ...
    if (status.dwCurrentState != SERVICE_RUNNING) {
      BOOST_LOG(log_error) << SERVICE_NAME " failed to start: "sv << status.dwWin32ExitCode;
      return false;
    }
  ...
  bool wait_for_ui_ready() {
    ...
    for (int i = 0; i < 30; i++) {
      ...
      if (err != NO_ERROR) {
        BOOST_LOG(log_error) << "Failed to query TCP table: "sv << err;
        return false;
      }
