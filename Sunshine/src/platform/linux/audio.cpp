...
namespace platf {
  ...
  struct mic_attr_t: public mic_t {
    ...
    capture_e sample(std::vector<float> &sample_buf) override {
      ...
      if (pa_simple_read(mic.get(), buf, sample_size * sizeof(float), &status)) {
        BOOST_LOG(log_error) << "pa_simple_read() failed: "sv << pa_strerror(status);
  ...
  std::unique_ptr<mic_t> microphone(const std::uint8_t *mapping, int channels, std::uint32_t sample_rate, std::uint32_t frame_size, std::string source_name) {
    ...
    if (!mic->mic) {
      ...
      BOOST_LOG(log_error) << "pa_simple_new() failed: "sv << err_str;
      return nullptr;
    }
  ...
  namespace pa {
    ...
    public:
      ...
      int init() {
        ...
        if (status) {
          BOOST_LOG(log_error) << "Couldn't connect to pulseaudio: "sv << pa_strerror(status);
          return -1;
        }
        ...
        worker = std::thread {
          [](loop_t::pointer loop) {
            ...
            if (status < 0) {
              BOOST_LOG(log_error) << "Couldn't run pulseaudio main loop"sv;
              return;
            }
          },
          loop.get()
      ...
      int unload_null(std::uint32_t i) {
        ...
        if (*alarm->status()) {
          BOOST_LOG(log_error) << "Couldn't unload null-sink with index ["sv << i << "]: "sv << pa_strerror(pa_context_errno(ctx.get()));
          return -1;
        }
      ...
      std::optional<sink_t> sink_info() override {
        ...
        cb_t<pa_sink_info *> f = [&](ctx_t::pointer ctx, const pa_sink_info *sink_info, int eol) {
          if (!sink_info) {
            if (!eol) {
              BOOST_LOG(log_error) << "Couldn't get pulseaudio sink info: "sv << pa_strerror(pa_context_errno(ctx));
        ...
        if (!op) {
          BOOST_LOG(log_error) << "Couldn't create card info operation: "sv << pa_strerror(pa_context_errno(ctx.get()));
      ...
      std::string get_default_sink_name() {
        ...
        cb_simple_t<pa_server_info *> server_f = [&](ctx_t::pointer ctx, const pa_server_info *server_info) {
          if (!server_info) {
            BOOST_LOG(log_error) << "Couldn't get pulseaudio server info: "sv << pa_strerror(pa_context_errno(ctx));
            alarm->ring(-1);
          }
      ...
      std::string get_monitor_name(const std::string &sink_name) {
        ...
        cb_t<pa_sink_info *> sink_f = [&](ctx_t::pointer ctx, const pa_sink_info *sink_info, int eol) {
          if (!sink_info) {
            if (!eol) {
              BOOST_LOG(log_error) << "Couldn't get pulseaudio sink info for ["sv << sink_name
      ...
      int set_sink(const std::string &sink) override {
        ...
        if (!op) {
          BOOST_LOG(log_error) << "Couldn't create set default-sink operation: "sv << pa_strerror(pa_context_errno(ctx.get()));
          return -1;
        }

        alarm->wait();
        if (*alarm->status()) {
          BOOST_LOG(log_error) << "Couldn't set default-sink ["sv << sink << "]: "sv << pa_strerror(pa_context_errno(ctx.get()));
