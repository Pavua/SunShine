...
namespace platf {
  ...
  bp::child run_command(bool elevated, bool interactive, const std::string &cmd, boost::filesystem::path &working_dir, const bp::v1::environment &env, FILE *file, std::error_code &ec, bp::group *group) {
    // clang-format off
    if (!group) {
      if (!file) {
  ...
  void open_url(const std::string &url) {
    ...
    bp::v1::environment _env = boost::this_process::environment();
    std::error_code ec;
