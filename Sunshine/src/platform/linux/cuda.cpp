...
namespace cuda {
  ...
  void pass_error(const std::string_view &sv, const char *name, const char *description) {
    BOOST_LOG(log_error) << sv << name << ':' << description;
  }
  ...
  int init() {
    ...
    if (status) {
      BOOST_LOG(log_error) << "Couldn't load cuda: "sv << status;
  ...
  public:
    ...
    int set_frame(AVFrame *frame, AVBufferRef *hw_frames_ctx) override {
      ...
      if (hwframe_ctx->sw_format != AV_PIX_FMT_NV12) {
        BOOST_LOG(log_error) << "cuda::cuda_t doesn't support any format other than AV_PIX_FMT_NV12"sv;
        return -1;
      }

      if (!frame->buf[0]) {
        if (av_hwframe_get_buffer(hw_frames_ctx, frame, 0)) {
          BOOST_LOG(log_error) << "Couldn't get hwframe for NVENC"sv;
          return -1;
        }
      }
  ...
  file_t open_drm_fd_for_cuda_device(int index) {
    ...
    try {
      ...
      BOOST_LOG(log_error) << "Failed to read sysfs: "sv << err.what();
    }

    BOOST_LOG(log_error) << "Unable to find DRM device with PCI bus ID: "sv << pci_bus_id.data();
    return -1;
  }
  ...
  public:
    ...
    int init(int in_width, int in_height, int offset_x, int offset_y) {
      ...
      if (file.el < 0) {
        char string[1024];
        BOOST_LOG(log_error) << "Couldn't open DRM FD for CUDA device: "sv << strerror_r(errno, string, sizeof(string));
        return -1;
      }

      gbm.reset(gbm::create_device(file.el));
      if (!gbm) {
        BOOST_LOG(log_error) << "Couldn't create GBM device: ["sv << util::hex(eglGetError()).to_string_view() << ']';
        return -1;
      }
    ...
    int set_frame(AVFrame *frame, AVBufferRef *hw_frames_ctx_buf) override {
      ...
      if (!frame->buf[0]) {
        if (av_hwframe_get_buffer(hw_frames_ctx_buf, frame, 0)) {
          BOOST_LOG(log_error) << "Couldn't get hwframe for VAAPI"sv;
