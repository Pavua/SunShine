...
#pragma clang diagnostic ignored "-Wtautological-pointer-compare"
    if ([[NSProcessInfo processInfo] isOperatingSystemAtLeastVersion:((NSOperatingSystemVersion) {10, 15, 0})] &&
      ...
      BOOST_LOG(log_error) << "No screen capture permission!"sv;
      BOOST_LOG(log_error) << "Please activate it in 'System Preferences' -> 'Privacy' -> 'Screen Recording'"sv;
...
#pragma clang diagnostic pop
  ...
  std::unique_ptr<deinit_t> enable_socket_qos(uintptr_t native_socket, boost::asio::ip::address &address, uint16_t port, qos_data_type_e data_type, bool dscp_tagging) {
    ...
    switch (data_type) {
      ...
      case qos_data_type_e::audio:
        ...
        break;
      default:
        BOOST_LOG(log_error) << "Unknown traffic type: "sv << (int) data_type;
        return nullptr;
    }
    ...
    if (service_type) {
      if (setsockopt(sockfd, SOL_SOCKET, SO_NET_SERVICE_TYPE, &service_type, sizeof(service_type)) == 0) {
        ...
        BOOST_LOG(log_error) << "Failed to set SO_NET_SERVICE_TYPE: "sv << errno;
      }
    }
    ...
    if (dscp_tagging) {
      ...
      if (dscp) {
        ...
        if (setsockopt(sockfd, level, option, &dscp, sizeof(dscp)) == 0) {
          ...
          BOOST_LOG(log_error) << "Failed to set TOS/TCLASS: "sv << errno;
        }
      }
    }
  ...
  std::string get_host_name() {
    try {
      ...
      BOOST_LOG(log_error) << "Failed to get hostname: "sv << err.what();
      return "Sunshine"s;
    }
  }
  ...
  int load(void *handle, const std::vector<std::tuple<apiproc *, const char *>> &funcs, bool strict) {
    ...
    for (auto &func : funcs) {
      ...
      *fn = (void (*)()) dlsym(handle, name);
      ...
      if (!*fn && strict) {
        BOOST_LOG(log_error) << "Couldn't find function: "sv << name;
