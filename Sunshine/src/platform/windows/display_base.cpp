...
namespace platf::dxgi {
  ...
  int duplication_t::init(display_base_t *display, const ::video::config_t &config) {
    ...
    // FIXME: Duplicate output on RX580 in combination with DOOM (2016) --> BSOD
    {
      ...
      if (SUCCEEDED(status)) {
        ...
        if (FAILED(status)) {
          BOOST_LOG(log_error) << "Failed to query IDXGIOutput1 from the output"sv;
          return -1;
        }
        ...
        if (FAILED(status)) {
          BOOST_LOG(log_error) << "DuplicateOutput Failed [0x"sv << util::hex(status).to_string_view() << ']';
          return -1;
        }
      }
    }
  ...
  capture_e duplication_t::next_frame(DXGI_OUTDUPL_FRAME_INFO &frame_info, std::chrono::milliseconds timeout, resource_t::pointer *res_p) {
    ...
    switch (status) {
      ...
      default:
        BOOST_LOG(log_error) << "Couldn't acquire next frame [0x"sv << util::hex(status).to_string_view();
  ...
  capture_e duplication_t::release_frame() {
    ...
    switch (status) {
      ...
      default:
        BOOST_LOG(log_error) << "Error while releasing duplication frame [0x"sv << util::hex(status).to_string_view();
        return capture_e::error;
    }
  }
  ...
  bool test_dxgi_duplication(adapter_t &adapter, output_t &output, bool enumeration_only) {
    ...
    if (FAILED(status)) {
      BOOST_LOG(log_error) << "Failed to create D3D11 device for DD test [0x"sv << util::hex(status).to_string_view() << ']';
      return false;
    }
    ...
    BOOST_LOG(log_error) << "DuplicateOutput() test failed [0x"sv << util::hex(status).to_string_view() << ']';
    return false;
  }
  ...
  int display_base_t::init(const ::video::config_t &config, const std::string &display_name) {
    ...
    if (FAILED(status)) {
      BOOST_LOG(log_error) << "Failed to create DXGIFactory1 [0x"sv << util::hex(status).to_string_view() << ']';
      return -1;
    }
    ...
    if (!output) {
      BOOST_LOG(log_error) << "Failed to locate an output device"sv;
      return -1;
    }
    ...
    if (FAILED(status)) {
      BOOST_LOG(log_error) << "Failed to query IDXGIAdapter interface"sv;
      return -1;
    }
    ...
    if (FAILED(status)) {
      BOOST_LOG(log_error) << "Failed to create D3D11 device [0x"sv << util::hex(status).to_string_view() << ']';
      ...
      if (gdi32) {
        auto check_hags = [&](const LUID &adapter) -> bool {
          ...
          if (!d3dkmt_open_adapter || !d3dkmt_query_adapter_info || !d3dkmt_close_adapter) {
            BOOST_LOG(log_error) << "Couldn't load d3dkmt functions from gdi32.dll to determine GPU HAGS status";
            return false;
          }

          D3DKMT_OPENADAPTERFROMLUID d3dkmt_adapter = {adapter};
          if (FAILED(d3dkmt_open_adapter(&d3dkmt_adapter))) {
            BOOST_LOG(log_error) << "D3DKMTOpenAdapterFromLuid() failed while trying to determine GPU HAGS status";
            return false;
          }
          ...
          if (FAILED(d3dkmt_close_adapter(&d3dkmt_close_adapter_wrap))) {
            BOOST_LOG(log_error) << "D3DKMTCloseAdapter() failed while trying to determine GPU HAGS status";
          }
        ...
        if (d3dkmt_set_process_priority) {
          ...
          BOOST_LOG(log_error) << "Couldn't load D3DKMTSetProcessSchedulingPriorityClass function from gdi32.dll to adjust GPU priority";
        }
      }
    ...
    // Try to reduce latency
    {
      ...
      status = device->QueryInterface(IID_IDXGIDevice, (void **) &dxgi);
      if (FAILED(status)) {
        BOOST_LOG(log_error) << "Failed to query DXGI interface from device [0x"sv << util::hex(status).to_string_view() << ']';
        return -1;
      }
    ...
    if (!timer || !*timer) {
      BOOST_LOG(log_error) << "Uninitialized high precision timer";
      return -1;
    }
