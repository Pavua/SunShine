...
namespace platf::dxgi {
  ...
  capture_e display_ddup_ram_t::snapshot(const pull_free_image_cb_t &pull_free_image_cb, std::shared_ptr<platf::img_t> &img_out, std::chrono::milliseconds timeout, bool cursor_visible) {
    ...
    if (frame_info.PointerShapeBufferSize > 0) {
      ...
      if (FAILED(status)) {
        BOOST_LOG(log_error) << "Failed to get new pointer shape [0x"sv << util::hex(status).to_string_view() << ']';
    ...
    if (frame_update_flag) {
      {
        ...
        if (FAILED(status)) {
          BOOST_LOG(log_error) << "Couldn't query interface [0x"sv << util::hex(status).to_string_view() << ']';
          return capture_e::error;
        }
        ...
        if (capture_format == DXGI_FORMAT_UNKNOWN) {
          ...
          if (FAILED(status)) {
            BOOST_LOG(log_error) << "Failed to create staging texture [0x"sv << util::hex(status).to_string_view() << ']';
            return capture_e::error;
          }
        }
    ...
    if (capture_format == DXGI_FORMAT_UNKNOWN) {
      ...
      if (FAILED(status)) {
        BOOST_LOG(log_error) << "Failed to map texture [0x"sv << util::hex(status).to_string_view() << ']';
  ...
  int display_ram_t::complete_img(platf::img_t *img, bool dummy) {
    ...
    if (!dummy && capture_format == DXGI_FORMAT_UNKNOWN) {
      BOOST_LOG(log_error) << "display_ram_t::complete_img() called with unknown capture format!";
      return -1;
    }
